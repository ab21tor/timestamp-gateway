...
**The double-bill crash edge (deliberate).** The receipt-before-save ordering has one operator-visible failure, and it is kept on purpose. A stamper crash between the receipt append and the calendar save leaves those commitments unsaved, so on restart the fork re-anchors them in a new transaction — and that second receipt carries a different txid, which the txid dedupe rightly treats as a new anchor. What you see is two bills in `GET /anchor-bills`: different txids, `confirmed_at` values close together, each carrying a `records` count for substantially the same batch of submissions, each priced records × `PER_RECORD_SATS`. The first of the two is for an anchor the calendar never saved, so its attestation is not what the proofs carry; the second is. The trade is deliberate and stays: a crash the other way around loses the receipt silently — revenue nothing would ever surface — so the failure is traded toward the extra bill everyone can see. The gateway neither detects nor reverses it; the refund is the operator's decision, and this is how the client makes the case.

How a client reconciles: keep a count of records submitted per anchor window (the `api-endpoint` ledger does this per fingerprint — its purchase events between two anchor confirmations are that window's records). When two bills whose `confirmed_at` values sit within one restart of each other together exceed the records submitted in that window, the excess is the duplicate: present both txids to the operator and the smaller bill is refunded. The paid-txid ledger in `pay-anchor-bills.sh` does not catch this case — the second bill is a genuinely new txid — the `records` arithmetic is what exposes it.
